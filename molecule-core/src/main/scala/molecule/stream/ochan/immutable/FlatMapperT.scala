...
case class FlatMapperT[A, B: Message](complexity: Int, f: B => Seg[A]) extends Transformer[A, B] { outer =>
  ...
  def apply(ochan: OChan[A]): OChan[B] = new OChan[B] {
    ...
    def add[C: Message](transformer: Transformer[B, C]): OChan[C] =
      transformer match {
        case MapperT(c, e) =>
          new FlatMapperT[A, C](complexity + c, e andThen f).apply(ochan)
        case FlatMapperT(c, e) =>
          new FlatMapperT[A, C](complexity + c, e andThen (_.flatMap(f))).apply(ochan)
        case _ =>
          transformer(this)
      }
  }
