...
// from https://github.com/twitter/bijection/blob/develop/project/DocGen.scala
object Site {

  val docDirectory = "target/site"
  val aggregateName = "molecule"
  ...
  lazy val unidocSettings: Seq[sbt.Setting[_]] =
    site.includeScaladoc(docDirectory) ++ Seq(
      scalacOptions in doc <++= (version, baseDirectory in LocalProject(aggregateName)).map { (v, rootBase) =>
        ...
        val docSourceUrl = "https://github.com/molecule-labs/" + aggregateName + "/tree/" + tagOrBranch + "€{FILE_PATH}.scala"
        Seq("-sourcepath", rootBase.getAbsolutePath, "-doc-source-url", docSourceUrl)
      },
      Unidoc.unidocDirectory := file(docDirectory),
      gitRemoteRepo := "git@github.com:molecule-labs/" + aggregateName + ".git",
      ghkeys.synchLocal <<= syncLocal
    )

  lazy val settings = site.settings ++ Unidoc.settings ++ ghpages.settings ++ unidocSettings
}